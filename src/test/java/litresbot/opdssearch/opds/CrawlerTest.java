...
public class CrawlerTest {
  ...
  public void testDownloadPageOk() throws IOException {
    ...
    when(mockHttp.sendGetRequest(anyString())).thenReturn(searchResult);
  ...
  public void testDownloadCatalogOk() throws IOException {
    ...
    Assert.assertEquals(4, pages.get(0).links.size());
  }

  // OPDS will be deprecated so:
  // no test for iterating over catalog pages
  // no test for cyclic links
  // no test for bad catalog data
}
